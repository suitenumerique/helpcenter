...
import { customTags, htmlComponents } from "./components";
...
export async function getDocument(
  ...
  const pad = `[\\s\\u200B-\\u200D\\uFEFF]*`;
  customTags.forEach((component) => {
    if (!(component in htmlComponents)) return;
    // Match opening and closing tags in separate <p> blocks (with optional content between)
    document.content = document.content.replace(
      new RegExp(
        `<p>${pad}&lt;${component}&gt;${pad}<\/p>(.*?)<p>${pad}&lt;\/${component}&gt;${pad}<\/p>`,
        "gs",
      ),
    ...
    document.content = document.content.replace(
      new RegExp(
        `<p>${pad}&lt;${component}&gt;${pad}&lt;\/${component}&gt;${pad}<\/p>`,
        "g",
      ),
      `<${component}></${component}>`,
    );
